# %%
"""
s3 counts description lengths weekly
------------------------------------

Measures the approximate length of descriptions in the 
'most_recent_jobs' S3 bucket, grouped by the "job posted" 
date in the main body of the HTML.

Creates a box plot showing the distribution of lengths each week.
...
def make_plot_data(test=False):
    ...
    descriptions_by_week = defaultdict(lambda: {'total': 0,  'approx_no_of_words': []})
    
    for one_advert in job_ads:
    ...
    plot_data['total_adverts'] = sum([data['total'] for data in descriptions_by_week.values()])
