...
def create_assign_dict_list(manual_assign):
    ...
    return [{'entity': e, 'surface_form': s, 'surface_form_type': 'manual'} for s, e in manual_assign]
...
manual_keep += manual_review_results[manual_review_results.manual_OK==3].surface_form.to_list()
...
# Additional list from the manual review
manual_remove_all = manual_remove + manual_remove_unsure + manual_review_results[manual_review_results.manual_OK==1].surface_form.to_list()
# Order and deduplicate
...
manual_assign_df = (pd.DataFrame(manual_assign, columns=['surface_form', 'id'])
                    .merge(skills[['preferred_label', 'id']], how='left')
                   )
