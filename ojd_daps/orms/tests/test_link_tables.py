...
from ojd_daps.orms.link_tables import JobAdDuplicateLink, JobAdJobTitleLink,\
    JobAdSOCLink, JobAdSICLink, JobAdESCOLink, JobAdSkillLink, JobAdTTWALink,\
    JobAdNUTS3Link, JobAdStdSalaryLink, JobAdExperienceLink, JobAdQualificationsLink,\
    JobAdReedAdDetailLink, JobAdLocationLink


@pytest.mark.timeout(10)
def test_job_ad_duplicate_link():
    Base = get_declarative_base(JobAdDuplicateLink)
    data = [{'first_id':'837456',
    'second_id':'3684',
    'weight':'0.99'}]
...
def test_job_ad_job_title_link():
    ...
    data = [{'job_id':'837456',
    'job_data_source':'3684',
    'std_job_title_id':'93409'}]
    # Insert the data in one transaction
    with db_session() as session:
        engine = session.get_bind()
        Base.metadata.create_all(engine)
