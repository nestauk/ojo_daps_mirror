"""
salaries.regex.single_regex_flow
-------------------
...
Run this flow with
    ...
    python single_regex_flow.py run
"""
from ojd_daps.flows.enrich.labs.salaries.regex.single_regex_utils import save_model
...
class RegexFlow(FlowSpec, DapsFlowMixin):
    ...
    picker_name = Parameter(
        "picker_name",
        help="The picker name, in case multiple salaries are found",
        default="max",
    )
    ...
    def save_model(self):
        ...
        """
        save_model(regex=self.regex, picker_name=self.picker_name)
        self.next(self.end)
