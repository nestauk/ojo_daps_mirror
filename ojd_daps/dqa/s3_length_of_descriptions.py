...
def count_approx_words(description):
    ...
    descrip_no_punc = descrip_stripped.translate(str.maketrans("","", string.punctuation))
...
# %%
_, job_ads = get_job_ads_posted_date(job_board=JOB_BOARD, sample_ratio=SAMPLE_RATIO, get_body=True)
...
# To capture results
descriptions_by_week = defaultdict(lambda: {'total': 0, 'missing': 0, 'approx_no_of_words': []})
...
total_with_description = sum([data['total']-data['missing'] for data in descriptions_by_week.values()])
total_percent_with_description = 100*total_with_description/total_adverts
print("Percentage of adverts with a description is {0:.0f} percent".format(total_percent_with_description))
...
# %%
# For the line chart
plot_data['percent_with_description'] = [data['percent_with_description'] for data in descriptions_by_week.values()]    

# For the box plot
plot_data['approx_no_of_words'] = [data['approx_no_of_words'] for data in descriptions_by_week.values()]    
...
ax.set_title(("Percentage of adverts with \n descriptions " 
        "by week posted \n (overall: {0:.0f}% "
...
fig, ax = plt.subplots(figsize=(20, 6))
ax.set_title(("Approximate number of words in advert descriptions " 
        " \n by week posted, based on {0} adverts".format(
    plot_data['total_adverts']))
    , fontsize=20)
