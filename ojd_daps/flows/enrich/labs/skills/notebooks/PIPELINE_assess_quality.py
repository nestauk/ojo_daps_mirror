...
def create_skills_documents(
    fpath=DATA_PATH / "interim/skills_docs.json", test=True, overwrite=False
):
    ...
    if os.path.exists(fpath) and not overwrite:
        ...
        # Preprocess the text
        skills_docs = {
            str(skills.iloc[i].id): clean_text(s)
            for i, s in enumerate(document_generator)
        }
        # Save the documents
...
def tfidf_phrase_sums(phrase, vocabulary, tfidf_matrix):
    ...
    return np.array(tfidf_matrix[:, token_indexes].sum(axis=1)).flatten().tolist()
...
def tfidf_phrase_means(phrase, vocabulary, tfidf_matrix):
    ...
    if len(token_indexes) > 0:
        return np.array(tfidf_matrix[:, token_indexes].mean(axis=1)).flatten().tolist()
    else:
...
def get_best_embedding_similarities(
    surface_form_df, precomputed=None, bert_transformer=None
):
    ...
    if precomputed is None:
        ...
        emb_sf = np.load(
            DATA_PATH
            / f"processed/embeddings/surface_forms_{precomputed}_emb_{BERT_MODEL}.npy"
        )
    ...
    for i in range(len(surface_form_df)):
        try:
            ...
            sim = (
                1
                - cdist(
                    emb_sf[i, :].reshape(1, -1),
                    emb_labels[vector_indexes, :],
                    metric="cosine",
                )
            ).max()
            sims.append(sim)
...
def create_numerical_features(
    inputs, precomputed=None, bert_transformer=BERT_TRANSFORMER
):
    ...
    if "score_emb_similarity" in NUMERICAL_FEATURES:
        model_data["score_emb_similarity"] = get_best_embedding_similarities(
            model_data, precomputed=precomputed, bert_transformer=bert_transformer
        )
    ...
    if "score_tfidf_sum" in NUMERICAL_FEATURES:
        model_data["score_tfidf_sum"] = get_tfidf_scores(
            model_data, vocabulary, tfidf_matrix, tfidf_phrase_sums
        )

    if "score_tfidf_mean" in NUMERICAL_FEATURES:
        model_data["score_tfidf_mean"] = get_tfidf_scores(
            model_data, vocabulary, tfidf_matrix, tfidf_phrase_means
        )
    ...
    if "score_idf_sum" in NUMERICAL_FEATURES:
        model_data["score_idf_sum"] = get_idf_scores(
            model_data, tfidf_vectorizer, idf_phrase_sums
        )

    if "score_idf_mean" in NUMERICAL_FEATURES:
        model_data["score_idf_mean"] = get_idf_scores(
            model_data, tfidf_vectorizer, idf_phrase_means
        )
    ...
    if os.path.exists(filepath) and not overwrite:
        ...
        if not test:
            ...
            df = (
                ...
                .sample(500)
                .append(skills_model["surface_forms"].iloc[-50:], ignore_index=True)
            )
        surface_forms_with_features = create_numerical_features(
            df, precomputed=skills_model["name"]
        )
...
def preprocessing_pipeline():
    ...
    categorical_transformer = OneHotEncoder(
        handle_unknown="ignore", categories=[CATEGORICAL_FEATURES_LIST]
    )
...
def train_manual_review_rating_model(skills_model, test=False):
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, random_state=999, test_size=0.2, train_size=0.8
    )
    ...
    print(
        f"Best threshold is {thresh_train['best_threshold']:.2f} (F1={thresh_train['best_F1']: .2f})"
    )
    evaluate_performance(
        X_train, y_train, clf.predict(X_train), thresh_train["best_threshold"]
    )
    print(f"Test set: (n={X_test.shape[0]})")
    thresh_test = find_best_threshold(clf, X_test, y_test)
    print(
        f"Best threshold is {thresh_test['best_threshold']:.2f} (F1={thresh_test['best_F1']: .2f})"
    )
    evaluate_performance(
        X_test, y_test, clf.predict(X_test), thresh_test["best_threshold"]
    )
    print("----")
    ...
    print("----")
    print(
        f"Evaluation on the combined training and test set: (n={X_transformed.shape[0]})"
    )
    print(
        f"Best threshold is {thresh['best_threshold']:.2f} (F1={thresh['best_F1']: .2f})"
    )
    evaluate_performance(
        X_transformed, y, clf.predict(X_transformed), thresh_train["best_threshold"]
    )
    print("----")

    # Apply the model on the full dataset
    surface_forms_with_predictions = get_prediction_probability(
        model_data, preprocessor, clf
    )
    surface_forms_with_predictions = get_prediction_discrete(
        surface_forms_with_predictions, t=thresh["best_threshold"]
    )
    ...
    new_model["surface_forms"] = (
        new_model["surface_forms"]
        .merge(
            surface_forms_with_predictions[
                ["entity", "surface_form", "predicted", "is_predicted_OK"]
            ],
            how="left",
...
if __name__ == "__main__":
    new_model, quality_rating_model = train_manual_review_rating_model(
        skills_model, test=False
    )
    ...
    print(
        classification_report(
            training_data[TARGET_COLUMN], df.is_predicted_OK.to_list()
        )
    )
