...
def get_all_surface_form_ids(detected_surface_forms, sf_to_id):
    detected_surface_form_ids = [
        [sf_to_id[sf] for sf in sf_list] for sf_list in detected_surface_forms
    ]
...
def get_surface_form_counts(detected_surface_forms):
    ...
    sf_counts_df = pd.DataFrame(
        data={
            "surface_form": sf_counts.keys(),
            "counts": sf_counts.values(),
            "fraction": np.array(list(sf_counts.values()))
            / len(detected_surface_forms),
        }
    )
...
def get_reviewed_ESCO_transversal_skills():
    ...
    return (
        pd.read_csv(DATA_PATH / "aux/general_skills_qual_checked.csv").query(
            "manual_is_general_skill==1"
        )
    ).id.to_list()
...
def build_cooc_graph(detected_surface_forms, unique_sf_ids, id_to_sf):
    ...
    G.add_edges(
        ...
        attributes={"weight": [np.log(x) for x in list(edge_counts.values())]},
    )
...
def calculate_coreness(graph, weights=None, ignore_betweenness=False):
    ...
    centrality = graph.eigenvector_centrality(
        directed=False, scale=True, weights=weights
    )
...
def surface_form_measures(sf_df, detected_surface_forms):
    ...
    coreness, graph_measures = calculate_coreness(
        G, weights=None, ignore_betweenness=False
    )
    ...
    sf_measures["coreness_data"] = coreness
    sf_measures = sf_measures.merge(sf_counts_df, how="left")

    sf_measures["skill_class"] = "other"
    sf_measures.loc[
        sf_measures.entity.isin(general_qual_skills_ids), "skill_class"
    ] = "general_qual"
    sf_measures.loc[
        sf_measures.entity.isin(language_skills_ids), "skill_class"
    ] = "language"
...
def export_core_surface_forms_to_check(
    sf_measures, prct_to_check=99, filename="general_skills_quant"
):
    ...
    general_sf_to_check = sf_measures[
        (
            sf_measures.coreness_data
            >= np.percentile(sf_measures.coreness_data, prct_to_check)
        )
...
def find_and_review_core_skills(
    sf_df, detected_surface_forms, prct_to_check=99, filename="general_skills_quant"
):
...
def get_language_skills_surface_forms(sf_df):
    skills = get_esco_skills()
    language_skills_ids = get_language_skills(skills)
    language_skills_forms = sf_df[
        sf_df.entity.isin(language_skills_ids)
...
def get_general_skills_surface_forms():
    ...
    general_quant_skills_ids = general_sf_checked.query(
        "is_manual_general_skill==1"
    ).entity.to_list()

    general_skills_ids = list(set(general_quant_skills_ids + general_qual_skills_ids))
    general_skills_forms = sf_df[
        sf_df.entity.isin(general_skills_ids)
    ].surface_form.to_list()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DATA_PATH / "aux/general_skills_quant_checked.csv"):
        ...
        filepath = (
            DATA_PATH
            / "processed/detected_skills/job_ads_DEC2020_MAY2021_detected_skills_v01_r1.pickle"
        )
        job_skills = pickle.load(open(filepath, "rb"))

        # Export table for checking
        detected_surface_forms = get_detected_surface_form_lists(job_skills)
        find_and_review_core_skills(
            sf_df, detected_surface_forms, prct_to_check=PRCTILE, filename=FILENAME
        )

    else:
        # Collect all checks
        language_skills_ids, language_skills_forms = get_language_skills_surface_forms(
