...
def assign_skill_to_cluster(cluster_counts_list):
    df = (pd.DataFrame(cluster_counts_list)
          .T
          .rename(columns={0:'cluster', 1:'counts', 2: 'predicted_q'})
         )
    ...
    if len(df_clust) or (df_clust.iloc[0] > df_clust.iloc[1]):
        ...
        assigned_clust = df_clust[df_clust.weighted_counts==df_clust.weighted_counts.max()].cluster.to_list()
...
def assign_skills_to_clusters(cluster_counts_lists, level='cluster_1'):
    ...
    skill_clusters = pd.DataFrame(data={
...
if __name__ == "__main__":
    ...
    partitions_counts = (sf_df
                         .merge(sf_counts[['surface_form', 'counts']], on=['surface_form'])
                         .dropna(axis=0, subset=['cluster_1'])
                        )
    cluster_counts_lists = get_entity_cluster_counts_lists(partitions_counts, 'cluster_1')
    skill_clusters_1 = assign_skills_to_clusters(cluster_counts_lists)    
    
    # Assign second level labels
    partitions_counts_2 = (partitions_counts
                           .merge(skill_clusters_1, on='preferred_label', how='left')
                           .query("cluster_1==skill_cluster_1")
                          )
    cluster_counts_lists_2 = get_entity_cluster_counts_lists(partitions_counts_2, 'cluster_2')
    ...
    sf_df_skill_clusters.loc[sf_df_skill_clusters.cluster_1.isnull(), 'cluster_status'] = 'assigned'
    sf_df_skill_clusters.loc[(sf_df_skill_clusters.cluster_1.isnull()==False)
                             & (sf_df_skill_clusters.cluster_1!=sf_df_skill_clusters.skill_cluster_1), 'cluster_status'] = 'reassigned'
    ...
    sf_df_skill_clusters['cluster_0'] = sf_df_skill_clusters.cluster_1.apply(lambda x: assign_cluster_0(x))

    surface_forms_with_clusters = (skills_model['surface_forms']
                                   .drop(['cluster_1', 'cluster_2'], axis=1)
                                   .merge(
                                       sf_df_skill_clusters[['surface_form', 'cluster_0', 'cluster_1', 'cluster_2', 'cluster_status']], 
                                       on='surface_form', 
                                       how='left')    
                                  )   
    ...
    new_sf_df = surface_forms_with_clusters.merge(manual_review[['surface_form', 'manual_OK']], how='left')    
