...
# %%
categories, job_ads = get_job_ads_posted_date(job_board=JOB_BOARD, sample_ratio=SAMPLE_RATIO, n_max=10)
...
for isoweek, chunk in grouped_dates:
    plot_data['label'].append(tuple(reversed(isoweek)))  # Human-readable iso week label (week, year - rather than - year, week)
    plot_data['numeric'].append(isoweek_to_universal_week(isoweek))   # Weeks since year zero
...
ax.set_title("Number of files in 'most_recent_jobs/production/reed/'\n"
...
for weekday, weekday_chunk in grouped_dates:    
    ...
    for isomonth, month_chunk in grouped_chunk:
        ...
        weekday_data[weekday_name]['numeric'].append(isomonth_to_universal_month(isomonth))
...
# Other styling
ax.set_title("Cumulative files in 'most_recent_jobs/production/reed/'\n"
             f"(estimated from {int(100*SAMPLE_RATIO)}% sampling)", fontsize=20, pad=20)
_ = ax.set_xlabel("(Month, Year)", fontsize=20, labelpad=20)
# %%
