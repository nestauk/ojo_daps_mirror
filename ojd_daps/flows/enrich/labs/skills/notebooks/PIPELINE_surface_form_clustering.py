...
def create_clustering_dataset(sf_df, manual_review, detected_surface_forms):
    ...
    lang_skill_forms = sf_counts[
        sf_counts.surface_form.isin(lang_skills["language_surface_forms"])
    ].surface_form.to_list()
    gen_skills_forms = sf_counts[
        sf_counts.surface_form.isin(gen_skills["general_skills_forms"])
    ].surface_form.to_list()
    ...
    excluded_forms = sf_counts[
        sf_counts.surface_form.isin(sf_df.surface_form.to_list()) == False
    ].surface_form.to_list()
    ...
    sf_to_cluster = sf_to_cluster.sort_values("surface_form").merge(
        sf_df[["surface_form", "surface_form_type", "entity", "preferred_label"]],
        how="left",
    )
...
def surface_form_2_vec(detected_surface_forms, use_cached=True):
    ...
    if os.path.exists(fpath) and use_cached:
        ...
        # Build the model
        model = Word2Vec(
            sentences=detected_surface_forms,
            size=200,
            window=max_window,
            min_count=1,
            workers=4,
...
def add_preselected_clusters(partitions):
    ...
    partitions_full = pd.concat(
        [
            ...
            assign_manual_clusters(
                lang_skills["language_surface_forms"], LANG_CLUST_1, LANG_CLUST_2
            ),
            assign_manual_clusters(
                gen_skills["general_skills_forms"], GENERAL_CLUST_1, GENERAL_CLUST_2
            ),
        ]
    )
...
def update_skills_model(skills_model, surface_form_partitions_full):
    ...
    new_model_sf_df = (
        ...
        .merge(
            surface_form_partitions_full[
                ["entity", "surface_form", "level_1", "level_2"]
            ],
            how="left",
            on=["surface_form", "entity"],
        )
        .rename(columns={"level_1": "cluster_1", "level_2": "cluster_2"})
    )
...
def clustering_pipeline(sf_to_cluster, sim_matrix, clustering_params):
    ...
    partition_1 = cluster_utils.collect_subclusters(
        0, fpath, session_name, n_total=len(clust_ids)
    )

    ############
    print(
        cluster_utils.ConsensusClustering.describe_partition(
            partition_1.cluster.to_list()
        )
    )
    ############
    ...
    partition_2 = cluster_utils.collect_subclusters(
        1, fpath, session_name, n_total=len(clust_ids)
    )
    ...
    partitions = partitions.rename(
        columns={"cluster_x": "level_1", "cluster_y": "level_2"}
    )
    ...
    partitions = (
        ...
        .drop_duplicates(
            "clust_id", keep="first"
        )  # just in case (in principle shouldn't happen)
...
if __name__ == "__main__":
    ...
    # Select forms to cluster
    sf_to_cluster = create_clustering_dataset(
        sf_df, manual_review, detected_surface_forms
    )
    ...
    partitions_full = add_preselected_clusters(partitions).merge(
        skills_model["surface_forms"], how="left"
    )
