...
Size of files in the 'most_recent_jobs' S3 bucket, grouped
...
def make_plot_data(test=False):
    ...
    for advert in job_ads:
        ...
        plot_data = list(zip(dates, filesizes))
    return plot_data
...
def make_plot(plot_data, test=False):
    ...
    ax.scatter(x=x, y=y)
    ax.set_title(f"Size of files in 'most_recent_jobs/production/{JOB_BOARD}/'\n"
                 f"(estimated from {int(100*sample_ratio)}% sampling)", fontsize=20, pad=20)
    ax.set_xlabel("(Week number)", fontsize=20, labelpad=20)
    ax.set_ylabel("(Filesize)", fontsize=20, labelpad=20)
    return ax
