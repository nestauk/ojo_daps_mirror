...
def regex_model(outcode_regex, boilerplate_text):
    ...
    return lambda raw_location: raw_location.split(',')[0].\
        replace(clean_, '').lower().replace(' ', '_')\
...
save_model('[A-Z]{1,2}[0-9][0-9A-Z]?\s?', '[^\w\s]')  # <--- After all of my hard work, I'll save my model config
