...
def extract_salary(salary):
    ...
    salary_ext["rate"] = guess_rate(salary_ext["min_salary"] + salary_ext["max_salary"] / 2)  # for some the range is very large so use average
    salary_ext["min_annualised_salary"] = round(annualised_salary(salary_ext["min_salary"], salary_ext["rate"]), 2)
    salary_ext['max_annualised_salary'] = round(annualised_salary(salary_ext["max_salary"], salary_ext["rate"]), 2)
    return salary_ext
...
def regex_model(regex):
    ...
    return lambda raw_salary: [float(result) for result in re_.findall(raw_salary) if len(result) > 0]
