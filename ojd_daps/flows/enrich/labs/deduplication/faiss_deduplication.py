...
def read_data(data, ids, orm, id_field, chunksize=10000, max_chunks=None):
...
def download_vectors(orm, id_field, chunksize=10000, max_chunks=None):
...
with db_session("dev") as session:
    save_model()
    data, ids = download_vectors(
        JobAdDescriptionVector, "id", chunksize=10, max_chunks=1
    )
